...
if ($isAdmin && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['addRecord'])) {
    ...
    $taskDate = date('Y-m-d', $timestamp) . " 星期{$weekday}";
...
<body>
        ...
        <tbody>
            <?php foreach ($recentRecords as $index => $record): ?>
                ...
                <tr id="record-<?php echo $originalIndex; ?>">
                    <td>
                        <?php if ($isAdmin): ?>
                            <form method="post" enctype="multipart/form-data">
                                ...
                                <!-- 提取日期部分用于date输入 -->
                                <?php $datePart = date('Y-m-d', strtotime($record['taskDate'])); ?>
                                <input type="date" name="taskDate" value="<?php echo $datePart; ?>" required>
                                <?php
                                // 显示中文星期
                                $timestamp = strtotime($record['taskDate']);
                                $weekMap = ['日', '一', '二', '三', '四', '五', '六'];
                                $weekday = $weekMap[date('w', $timestamp)];
                                echo "<div>星期{$weekday}</div>";
                                ?>
                        <?php else: ?>
                    ...
                    <td>
                        <?php if ($isAdmin): ?>
                            <textarea name="taskName" rows="4" cols="50"><?php echo htmlspecialchars($record['taskName']); ?></textarea><br>
                        <?php else: ?>
                            <?php echo $record['taskNamePublic'] === '是' ? nl2br(htmlspecialchars($record['taskName'])) : '【内容隐藏】'; ?>
                        <?php endif; ?>
                    </td>
                    <?php if ($isAdmin): ?>
                        <td>
    ...
    <script>
        ...
        // 任务可见性数据
        const taskVisibility = <?php echo json_encode(array_map(function($record) {
            return [
                'namePublic' => $record['taskNamePublic'] === '是',
                'contentPublic' => $record['taskContentPublic'] === '是'
            ];
        }, $records)); ?>;
        ...
        function updateNavButtons() {
            ...
            for (let i = currentRecordIndex - 1; i >= 0; i--) {
                if (isAdmin || (taskVisibility[i] && taskVisibility[i].contentPublic)) {
                    hasPrevTask = true;
                    break;
                }
            }
            ...
            document.getElementById('prev-task').disabled = !hasPrevTask;
            document.getElementById('next-task').disabled = !hasNextTask;
        }
        ...
        function navigateMedia(direction) {
            ...
            // 检查当前任务是否对非管理员可见
            if (!isAdmin && (!taskVisibility[currentRecordIndex] || !taskVisibility[currentRecordIndex].contentPublic)) {
                return;
            }
            ...
            if (newMediaIndex >= 0 && newMediaIndex < mediaFiles.length) {
                openLightbox(mediaFiles[newMediaIndex].path, mediaFiles[newMediaIndex].type, currentRecordIndex, newMediaIndex);
            }
        }
        ...
        function navigateTask(direction) {
            ...
            // 计算新的任务索引
            let newRecordIndex = currentRecordIndex + direction;
            const totalRecords = mediaData.length;
            
            // 权限验证：找到下一个可见的任务
            while (newRecordIndex >= 0 && newRecordIndex < totalRecords) {
                // 管理员可以查看所有任务
                if (isAdmin) {
                    break;
                }
                // 非管理员只能查看内容公开的任务
                else if (taskVisibility[newRecordIndex] && taskVisibility[newRecordIndex].contentPublic) {
                    break;
                }
                // 否则继续查找下一个
