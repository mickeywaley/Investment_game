...
<body>
    ...
    <div id="lightbox">
        ...
        <button id="prev-task" class="task-nav-btn">&lt;</button>
        <button id="next-task" class="task-nav-btn">&gt;</button>
        <div id="lightbox-content">
            <!-- 图片或视频将在这里显示 -->
        </div>
        <div class="lightbox-nav">
            <button class="lightbox-btn" id="prev-media" disabled>上一张</button>
            <div class="lightbox-task-name" id="lightbox-task-name"></div>
            <button class="lightbox-btn" id="next-media" disabled>下一张</button>
        </div>
    ...
    <script>
        ...
        // 初始化可见记录数组
        function initVisibleRecords() {
            visibleRecords = [];
            allRecords.forEach((record, index) => {
                if (record.mediaFiles && record.mediaFiles.length > 0) {
                    if (!record.taskContentPublic || record.taskContentPublic === '是') {
                        visibleRecords.push({
                            index: index,
                            record: record
                        });
        ...
        function updateTaskNavButtons() {
            const currentVisibleIndex = visibleRecords.findIndex(item => item.index === currentRecordIndex);
            prevTaskButton.disabled = currentVisibleIndex <= 0;
            nextTaskButton.disabled = currentVisibleIndex >= visibleRecords.length - 1;
        }

        function goToPrevTask() {
            const currentVisibleIndex = visibleRecords.findIndex(item => item.index === currentRecordIndex);
            if (currentVisibleIndex > 0) {
                const prevTask = visibleRecords[currentVisibleIndex - 1];
                openLightbox(prevTask.record.mediaFiles, prevTask.index, prevTask.record.taskName, 0);
            }
        }

        function goToNextTask() {
            const currentVisibleIndex = visibleRecords.findIndex(item => item.index === currentRecordIndex);
            if (currentVisibleIndex < visibleRecords.length - 1) {
                const nextTask = visibleRecords[currentVisibleIndex + 1];
                openLightbox(nextTask.record.mediaFiles, nextTask.index, nextTask.record.taskName, 0);
            }
        }
        ...
        prevTaskButton.addEventListener('click', goToPrevTask);
