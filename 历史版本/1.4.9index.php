...
<body>
    ...
    <script>
        ...
        let isCtrlPressed = false; // 用于跟踪Ctrl键状态
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // 事件监听
            document.getElementById('lightbox').addEventListener('wheel', handleWheel);
        ...
        function handleWheel(e) {
            if (document.getElementById('lightbox').style.display === 'flex') {
                ...
                if (isCtrlPressed) {
                    if (e.deltaY > 0) {
                        navigateTask(1); // 下一个任务
                    } else {
                        navigateTask(-1); // 上一个任务
                    }
                } 
                // 普通滚轮 切换媒体
                else {
        ...
        function openLightbox(src, type, recordIndex, mediaIndex) {
            ...
            if (type === 'image') {
                ...
                img.src = src;
                img.onload = updateLightboxContent;
                ...
                video.controls = true;
                video.autoplay = true;
                video.onloadedmetadata = updateLightboxContent;
        ...
        function updateLightboxContent() {
            const mediaElement = document.querySelector('#lightbox-content img, #lightbox-content video');
            if (mediaElement) {
        ...
        function updateNavButtons() {
            const mediaCount = <?php echo json_encode(array_map(function($record) { 
                return count($record['mediaFiles']); 
            }, $records)); ?>[currentRecordIndex] || 0;
